...
const namedSheet = (value) => ({ __rl: true, value, mode: 'name' });
const jsLiteral = (value) => JSON.stringify(String(value));

function googleNode(id, name, position, operation, tab, extra = {}) {
  return {
    parameters: { operation, documentId, sheetName: namedSheet(tab), ...extra },
    id, name, type: 'n8n-nodes-base.googleSheets', typeVersion: 4.6, position,
    credentials: { googleApi: googleCredential },
  };
}

const scoringCode = [
  `const company=${jsLiteral(company)};`,
  `const ownerEmail=${jsLiteral(ownerEmail)};`,
  "const rows=$input.all().map(i=>i.json);",
  "const state=$getWorkflowStaticData('global');state.done=state.done||{};",
  "const pick=(r,ks)=>{for(const k of ks){const v=r[k];if(v!==undefined&&v!==null&&String(v).trim()!=='')return String(v).trim();}return '';};",
  "const num=v=>Number(String(v??'').replace(/[,，\\s]/g,''))||0;",
  "const has=(v,ws)=>ws.some(w=>String(v||'').includes(w));",
  ...
  "const name=pick(r,['姓名','Name']);const url=pick(r,['社交平台链接','平台链接','账号链接']);",
  "const submitted=pick(r,['提交时间','Timestamp','时间戳'])||day(now);const key=[submitted,name,url,i].join('|');",
  "if(!name||!url||pick(r,['n8n处理状态','自动化处理状态','处理状态'])==='已处理'||state.done[key])continue;",
  "const followers=num(pick(r,['粉丝数','Followers']));const views=num(pick(r,['平均播放量','近10条平均播放量','Avg Views']));const interactions=num(pick(r,['平均互动量','近10条平均互动量','Avg Engagements']));",
  "const type=pick(r,['内容类型','Content Type']);const identity=pick(r,['年级/身份','身份','年级']);const audience=pick(r,['受众画像','Audience']);const intention=pick(r,['合作意向','Collaboration Intention']);",
  "const experience=pick(r,['过往合作经验','合作经验']);const caseStudy=pick(r,['代表合作案例','案例']);const consent=pick(r,['是否同意后续联系','是否同意联系'])||'是';const price=num(pick(r,['报价','Price']));const rate=views?interactions/views:0;",
  "const fan=followers>=50000&&views>=10000?20:followers>=10000&&views>=3000?16:followers>=5000&&views>=1000?12:followers>=1000&&views>=300?8:4;",
  "const match=has(type,['AI视频','AI工具','视频剪辑','互动视频'])?25:has(type,['影视娱乐','短剧','科技数码','内容平台'])?20:has(type,['校园','留学','社群'])?16:has(type,['生活','美妆','穿搭','探店'])?10:5;",
  "const engage=rate>=.08?20:rate>=.05?16:rate>=.03?12:rate>=.01?8:4;",
  "const campus=has(identity+intention,['社群负责人','校园大使','学生组织'])?15:has(identity,['本科','研究生','留学生'])&&has(intention,['校园推广','社群分发'])?12:has(identity,['本科','研究生','留学生'])?9:has(audience,['大学生','校园','留学生'])?6:3;",
  "const cpv=views?price/views:999;const priceScore=cpv<=.05?10:cpv<=.1?8:cpv<=.2?6:cpv<=.5?4:2;",
  "const history=experience.includes('10次以上')&&caseStudy?10:experience.includes('4-10次')&&caseStudy?8:experience.includes('1-3次')?6:has(intention,['长期合作','拉新注册','任务挑战','校园推广'])?4:2;",
  "const total=fan+match+engage+campus+priceScore+history;const tier=total>=80?'A类':total>=60?'B类':'C类';const priority=tier==='A类'?'高':tier==='B类'?'中':'低';const status=tier==='C类'?'观察池':'待联系';const valid=['A类','B类'].includes(tier)&&consent!=='否'?'有效':'待观察';",
  "const next=new Date(now);if(tier==='B类')next.setDate(next.getDate()+1);if(tier==='C类')next.setDate(next.getDate()+7);",
  "const school=pick(r,['学校','School']);const region=pick(r,['地区','Region']);",
  "const recommended=tier==='A类'?'重点定制合作：内容共创、校园节点、长期大使、注册转化任务':tier==='B类'?'标准任务合作：产品体验、内容发布、社群分发、CPA测试':'观察池：低成本试用、后续活动邀请、暂缓高预算投入';",
  "const message=tier==='A类'?'你好 '+name+'，我是'+company+'合作负责人。我们正在招募AI视频与互动视频方向创作者，希望沟通定制内容共创或校园推广合作。':tier==='B类'?'你好 '+name+'，我是'+company+'合作负责人。想邀请你参与产品体验、内容发布或社群推广合作。':'你好 '+name+'，感谢关注'+company+'。我们会将你纳入创作者观察池，后续有匹配活动会优先联系。';",
  "const kolId='KOL-'+ymd+'-'+String(i+2).padStart(3,'0');const followId='FU-'+ymd+'-'+String(i+2).padStart(3,'0');state.done[key]=true;",
  "out.push({json:{...r,'KOL ID':kolId,'提交时间':submitted,'姓名':name,'联系邮箱':pick(r,['联系邮箱','邮箱','Email']),'联系方式':pick(r,['联系方式','微信号/其他联系方式','微信']),'学校':school,'年级/身份':identity,'地区':region,'主要社交平台':pick(r,['主要社交平台','社交平台','平台']),'社交平台链接':url,'粉丝数':followers,'平均播放量':views,'平均互动量':interactions,'互动率':rate,'内容类型':type,'受众画像':audience,'合作意向':intention,'报价':price,'可接受合作方式':pick(r,['可接受合作方式','合作方式']),'过往合作经验':experience,'代表合作案例':caseStudy,'是否同意后续联系':consent,'粉丝质量分':fan,'内容匹配度分':match,'互动率分':engage,'校园影响力分':campus,'报价合理性分':priceScore,'过往合作表现分':history,'总分':total,'KOL分层':tier,'推荐合作方式':recommended,'自动联系话术':message,'当前跟进状态':status,'优先级':priority,'负责人':ownerEmail,'下一次跟进日期':day(next),'是否有效KOL':valid,'n8n处理状态':'已处理','跟进ID':followId,'跟进方式':'自动生成','跟进内容':message,'下一步动作':recommended,'是否需要提醒':tier==='C类'?'否':'是','邮件主题':'【'+company+' KOL招募】'+name+' / '+tier+' / '+total+'分','邮件正文':name+' 已完成自动评分：'+total+'分，'+tier+'。\\n主页：'+url+'\\n推荐：'+recommended+'\\n话术：'+message}});}",
  "return out;",
].join('\n');
...
const weeklyCode = [
  "const rows=$input.all().map(i=>i.json);const total=rows.length;",
  "const valid=rows.filter(r=>r['是否有效KOL']==='有效').length;const a=rows.filter(r=>r['KOL分层']==='A类').length;const b=rows.filter(r=>r['KOL分层']==='B类').length;const c=rows.filter(r=>r['KOL分层']==='C类').length;",
  "const converted=rows.filter(r=>['已合作','已签约','合作中'].includes(String(r['当前跟进状态']||''))).length;const regs=rows.reduce((s,r)=>s+(Number(r['注册数'])||0),0);const tasks=rows.reduce((s,r)=>s+(Number(r['任务完成数'])||0),0);",
  `return [{json:{subject:'【${company} KOL周报】自动化数据看板摘要',body:'KOL招募周报\\n提交/入库：'+total+'\\n有效KOL：'+valid+'\\nA类：'+a+'\\nB类：'+b+'\\nC类：'+c+'\\n已合作/签约/合作中：'+converted+'\\n注册数：'+regs+'\\n任务完成数：'+tasks}}];`,
].join('\n');
